...
| `SPM_HEDGE_AFTER_S` | `2` | when the chain has at least `SPM_HEDGE_MIN_CANDIDATES` free candidates, a silent live candidate given after the first one is dropped after this long and the walk moves on. A live candidate that is going to answer does so in well under a second (demo: ~550 ms end to end); a two-MAC chain sitting on the 12 s window per MAC was up to 24 s of black screen. `0` disables every kind. Not a parallel race - that would hold two panel slots for one zap, which is exactly what answers `limit` |
| `SPM_HEDGE_MIN_CANDIDATES` | `0` | how many MACs in the chain must be free before the fence is lowered (`0` = the whole chain must be free - the safe default). With nothing free to fall back to, patience is the only thing that helps, so the patient windows stay. The first candidate of a play is never fence-cut either: it is the source the engine believes in and gets its full window. Applies only to `SPM_HEDGE_KINDS` - decrease it, don't raise it, if a chained slow VOD starts failing |
| `SPM_HEDGE_KINDS` | `live` | which item kinds the hedge fence applies to. Live is where the 550 ms measurement came from; a VOD/episode first byte is measured in seconds (a storage the panel picked, a cold CDN edge), so hedge-cutting it to 2 s marked movies dead before their first byte (`no data within 2s … -> fallback` for every MAC, then 502) - the "VOD still not working" regression. Excluded by default |
...
1. **Portals** – add each Stalker portal base URL and its MAC addresses (optionally per-MAC password). *Check Portal* resolves the real endpoint (`/c/`, `/client/`, `/portal.php`, …) and verifies each MAC online (busy-ness and subscription expiry included); the per-MAC result now carries **why** a failure happened (`code` + the panel's own wording), not just "failed". Two per-portal network switches live in the same editor: **HTTP proxy** and **Allow broken TLS** (certificate verification is ON for every portal unless that box is ticked — a `TLS unverified` badge then marks the portal in the list, because it is a deliberate exception, not a setting to forget). *Delete* offers a replacement-dialog cleanup for playlists that reference it.
   ...
   **Multi-MAC health.** Portals with two or more MACs get a background sweep (Settings → *Multi-MAC status refresh*, default every 60 min; `0` pauses it) that handshakes every MAC and refreshes `status` / `online` / `expire_date` / `last_checked` — the same work *Check Portal* does, kept honest overnight. MACs currently occupied are skipped so a viewer is never kicked: that covers both ffmpeg-proxied plays (hard `mac_locks`) and redirect/direct plays (a soft lease after the 302, because once the player is sent to the panel CDN we no longer hold the socket). The Portals toolbar *Refresh MAC health* button runs the same sweep on demand. On a multi-MAC portal, **Compare genres across MACs** first lets the operator select which online accounts to contact, then renders Live/VOD/Series genre-by-MAC matrices with text, difference, coverage, MAC and package filters. The picker also carries the **channel-id second phase**: Live is always requested (1 request per MAC via `get_all_channels`), VOD and Series are opt-in checkboxes (paged per enabled genre, same page budget as a normal fetch). That phase runs **only when the genre diff finds >=2 distinct packages** — same channel name resolving to a different id on another MAC is what breaks a shared primary/fallback chain (one stored `cmd`, two id spaces), and on identical packages there is nothing to warn about. When conflicts are found, the Summary tab shows a red *"Channel ids differ across packages — do NOT chain those MACs"* badge and the matrix gains a **Channel IDs** tab (marked with a warning) listing each flagged channel with per-MAC id columns, an id-conflict/missing filter and per-kind counts; matching channels stay in the counts line only. Enabled genres are resolved **per MAC by id then name**, because packages can renumber genre ids. Exact matching signatures are grouped as packages; selected or visible stored genres can be enabled/disabled in bulk. Successful Live/VOD/Series genre counts and the comparison time are persisted per MAC and shown directly on the portal list (`Genres L … · V … · S …`); failed content-kind requests retain their previous count instead of being recorded as zero. The comparison **upserts the selected MACs' union into the portal's genre tables** (existing `enabled` flags are kept; brand-new genres land disabled). Useful when a "secondary" MAC is actually a different package from a shared-login reseller. Removing a MAC or deleting a portal also drops its runtime leftovers (mac locks, redirect leases, pooled Stalker sessions) — DB cascades already wipe the durable rows. When ids still differ after the compare, the Channel IDs tab's **Save N id translation(s)** button stores one row per MAC in `channel_id_translations` — the live source, the MAC, and that MAC's own channel id and cmd — so playback asks every MAC with its own id and the whole portal stays usable as primary/fallback; **Clear** in the same tab removes them again.

   Regression tests: `pytest tests/test_mac_health.py tests/test_channel_ids_gui.py` (the GUI check needs `node` and `npm install --prefix dev` for jsdom) and `node dev/check-js.js`.
