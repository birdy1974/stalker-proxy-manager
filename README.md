...
| `SPM_HEDGE_AFTER_S` | `2` | when at least `SPM_HEDGE_MIN_CANDIDATES` MACs in the chain are free to play, a candidate that has produced no byte after this long is dropped and the walk moves on. A candidate that is going to answer does so in well under a second (demo: ~550 ms end to end); a two-MAC chain sitting on the 12 s window per MAC was up to 24 s of black screen. `0` disables. Not a parallel race - that would hold two panel slots for one zap, which is exactly what answers `limit` |
| `SPM_HEDGE_MIN_CANDIDATES` | `2` | how many free candidates must exist before the fence is lowered. With one free MAC (a single-MAC portal) patience is the only thing that helps, so the patient windows stay |
